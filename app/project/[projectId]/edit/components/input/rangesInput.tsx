...
export const RangesInput: React.FC<RangesInputProps> = ({
  ...
  const updateXMin = (v: number) => {
    ...
    const next: Ranges = {
      ...ranges,
      xrange: [nextMin, ranges.xrange[1], ranges.xrange[2]],
    };
    setRanges(next);
    onChange(next);
    refreshFunc();
  ...
  const updateXMax = (v: number) => {
    ...
    const next: Ranges = {
      ...ranges,
      xrange: [ranges.xrange[0], nextMax, ranges.xrange[2]],
    };
    setRanges(next);
    onChange(next);
    refreshFunc();
  };
  const updateXStep = (v: number) => {
    const nextStep = v;
    const next: Ranges = {
      ...ranges,
      xrange: [ranges.xrange[0], ranges.xrange[1], nextStep],
    };
    setRanges(next);
    onChange(next);
    refreshFunc();
  ...
  const updateYMin = (v: number) => {
    ...
    const next: Ranges = {
      ...ranges,
      yrange: [nextMin, ranges.yrange[1], ranges.yrange[2]],
    };
    setRanges(next);
    onChange(next);
    refreshFunc();
  ...
  const updateYMax = (v: number) => {
    ...
    const next: Ranges = {
      ...ranges,
      yrange: [ranges.yrange[0], nextMax, ranges.yrange[2]],
    };
    setRanges(next);
    onChange(next);
    refreshFunc();
  };
  const updateYStep = (v: number) => {
    const nextStep = v;
    const next: Ranges = {
      ...ranges,
      yrange: [ranges.yrange[0], ranges.yrange[1], nextStep],
    };
    setRanges(next);
    onChange(next);
    refreshFunc();
        ...
        <PopoverContent className="w-80 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-mono">{ranges.xrange[2].toFixed(1)}</span>
              </div>

              <Slider
                value={[ranges.xrange[2]]}
                min={0.1}
                max={3}
                step={0.1}
                onValueChange={(v) => updateXStep(v[0])}
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs">
                <span className="text-muted-foreground">Step</span>
                <span className="font-mono">{ranges.yrange[2].toFixed(1)}</span>
              </div>

              <Slider
                value={[ranges.yrange[2]]}
                min={0.1}
                max={3}
                step={0.1}
                onValueChange={(v) => updateYStep(v[0])}
              />
            </div>
          </div>
