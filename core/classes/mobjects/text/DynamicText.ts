...
export class DynamicText extends MText {
  ...
  set dynamicContent(val: Partial<{ val1: number; val2: number }>) {
    this._dynamicContent.val1 = val.val1 || 0;
    this._dynamicContent.val2 = val.val2 || 0;
    const line = this.defaultText;
    const updatedLine = line
      .replace(
        /val1/g,
        val.val1?.toFixed(2).toString() || this._dynamicContent.val1.toString(),
      )
      .replace(
        /val2/g,
        val.val2?.toFixed(2).toString() || this._dynamicContent.val2.toString(),
      );
